  ...
  <section class="main">
    ...
    <section id="dailySales" class="view">
      ...
      <div class="panel"><div class="toolbar"><h2>Owner equity and drawings</h2><label>View owner<select id="ownerFilter"><option value="">All owners</option></select></label></div><div class="manual-row">
        <label>Owner name<input id="ownerName" list="ownerNameList" placeholder="Owner name"><datalist id="ownerNameList"></datalist></label>
...
function syncOwnerPicker(){let names=ownerNames(),filter=q("#ownerFilter"),list=q("#ownerNameList");if(list)list.innerHTML=names.map(n=>`<option value="${html(n)}"></option>`).join("");if(!filter)return;let selected=filter.value;filter.innerHTML='<option value="">All owners</option>'+names.map(n=>`<option value="${html(n)}">${html(n)}</option>`).join("");filter.value=names.includes(selected)?selected:""}
function renderOwnerMovements(bounds){syncOwnerPicker();let selected=q("#ownerFilter")?q("#ownerFilter").value:"",rows=ownerRows(bounds).filter(r=>!selected||String(r.name||"")===selected),el=q("#ownerMovements");if(!el)return;el.className=rows.length?'':'empty';el.innerHTML=rows.length?table(rows,['Date','Owner','Type','Method','Amount','Remark',''],r=>`<tr><td>${html(String(r.date||"").split("-").reverse().join("/"))}</td><td>${html(r.name)}</td><td>${html(r.type==="drawing"?"Drawings":"Owner Equity")}</td><td>${html(paymentLabel(r.method))}</td><td>RM ${asMoney(r.amount).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</td><td>${html(r.note||"")}</td><td><button class="danger" data-delete-owner="${html(r.id)}">Delete</button></td></tr>`):selected?`No owner equity or drawings for ${html(selected)} in this date range.`:'No owner equity or drawings for this date range.';qa("[data-delete-owner]").forEach(btn=>btn.onclick=()=>deleteOwnerMovement(btn.dataset.deleteOwner))}
