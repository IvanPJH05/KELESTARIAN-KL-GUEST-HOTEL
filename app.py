...
<main class="app">
  ...
  <section class="main">
    ...
    <section id="dailySales" class="view">
      <div class="panel"><div class="toolbar"><h2>Daily sales</h2></div><div class="filters"><button data-daily-range="today" class="active">Today</button><button data-daily-range="custom">Custom date</button><input id="dailyStartDate" type="date"><input id="dailyEndDate" type="date"></div></div>
      <div class="panel"><h2>Lestari and Deposit</h2><br><div id="lestariDeposit" class="empty">No manual transactions in this date range.</div></div>
      <div class="panel"><div class="toolbar"><h2>Transactions</h2></div><div id="dailySalesTable" class="empty">No manual transactions in this date range.</div></div>
    </section>
...
function renderLestariDeposit(rows){let manualRows=rows.filter(r=>!r.is_paid_continuation&&(r.flags||[]).includes("MANUAL_CHECK_IN")),lestari={cash:0,qr:0,transfer:0},deposit={cash:0,qr:0};manualRows.forEach(r=>{let lestariMethod=methodKey(r.kelestarian_payment_method),depositMethod=methodKey(r.deposit_payment_method);if(lestariMethod==="cash")lestari.cash+=asMoney(r.kelestarian);if(lestariMethod==="qr")lestari.qr+=asMoney(r.kelestarian);if(lestariMethod==="transfer")lestari.transfer+=asMoney(r.kelestarian);if(depositMethod==="cash"&&cashDepositActive(r))deposit.cash+=asMoney(r.deposit);if(depositMethod==="qr")deposit.qr+=asMoney(r.deposit)});let el=q("#lestariDeposit");if(!el)return;let lestariTotal=lestari.cash+lestari.qr+lestari.transfer,depositTotal=deposit.cash+deposit.qr;el.className=manualRows.length?'':'empty';el.innerHTML=manualRows.length?`<h2>Lestari Payment</h2><br><div class="cards">${moneyCard("Total Cash",lestari.cash)}${moneyCard("Total QR Payment",lestari.qr)}${moneyCard("Total Bank Transfer",lestari.transfer)}${moneyCard("Grand Total",lestariTotal)}</div><h2>Deposit</h2><br><div class="cards">${moneyCard("Total Cash Deposit",deposit.cash)}${moneyCard("Total QR Payment",deposit.qr)}${moneyCard("Grand Total",depositTotal)}</div>`:'No manual transactions in this date range.'}
function renderDailySales(){let rows=manualTransactionRows();renderLestariDeposit(rows);let el=q("#dailySalesTable");if(!el)return;el.className=rows.length?'':'empty';el.innerHTML=rows.length?table(rows,['Input time','Room','Registration','Stay type','Duration','Check in','Check out','Room payment','Kelestarian payment','Deposit payment','Remark'],r=>`<tr><td>${html(formatDateTime(r.input_time||flagValue(r.flags,"INPUT_TIME")))}</td><td>${html(r.room_no)}</td><td>${html(r.registration||flagValue(r.flags,"REGISTRATION"))}</td><td>${html(r.stay_type_label||flagValue(r.flags,"STAY_TYPE"))}</td><td>${html(r.stay_progress)}</td><td>${html(String(r.check_in_date||r.date).split("-").reverse().join("/"))}</td><td>${html(String(r.check_out_date||"").split("-").reverse().join("/"))}</td><td>RM ${r.price||"0.00"} (${html(r.payment_method||"")})</td><td>RM ${r.kelestarian||"0.00"} (${html(r.kelestarian_payment_method||"")})</td><td>RM ${r.deposit||"0.00"} (${html(r.deposit_payment_method||"")})</td><td>${html(r.remark||flagValue(r.flags,"REMARK"))}</td></tr>`):'No manual transactions in this date range.'}
