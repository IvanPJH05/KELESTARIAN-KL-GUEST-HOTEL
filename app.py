...
<main class="app">
  <aside class="sidebar">
    ...
    <select id="workspaceSelect" class="workspace-select" aria-label="Workspace">
      ...
      <option value="b">Laporan B</option>
      <option value="c">Laporan C</option>
    ...
    <nav class="nav">
      ...
      <button data-view="dailySales">Daily Sales</button>
  ...
  <section class="main">
    ...
    <section id="manual" class="view">
      ...
      <div class="panel"><div class="toolbar"><h2>Transactions</h2></div><div id="manualHistory" class="empty">No manual check-ins saved yet.</div></div>
    </section>
    <section id="dailySales" class="view">
      ...
      <div class="panel"><h2>Cash / QR / Bank on hand</h2><br><div id="dailyCashflow" class="empty">No check-ins in this date range.</div></div>
      <div class="panel"><div class="toolbar"><h2>Owner equity and drawings</h2></div><div class="manual-row">
        <label>Owner name<input id="ownerName" placeholder="Owner name"></label>
      ...
      </div><br><div id="ownerMovements" class="empty">No owner equity or drawings for this date range.</div></div>
      <div class="panel"><div class="toolbar"><h2>Transfer between Cash / QR / Bank</h2></div><div class="manual-row">
      ...
      </div><br><div id="transferMovements" class="empty">No transfers for this date range.</div></div>
...
function setReportDefaults(){let dates=sales.filter(r=>r.date&&r.date>='2026-01-01'&&r.date<='2026-12-31'&&!r.is_paid_continuation).map(r=>r.date).sort(),latest=dates.at(-1);if(!latest)return;let months=[...new Set(dates.map(d=>d.slice(0,7)))],latestMonth=latest.slice(0,7),savedB=localStorage.getItem("reportMonthB"),savedC=localStorage.getItem("reportMonthC"),cMode=localStorage.getItem("reportCMode")||"month";if(!months.includes(q("#reportMonthB").value))q("#reportMonthB").value=months.includes(savedB)?savedB:latestMonth;if(cMode!=="custom"){if(!months.includes(q("#reportMonthC").value))q("#reportMonthC").value=months.includes(savedC)?savedC:q("#reportMonthB").value;if(!q("#reportStartC").value||!q("#reportEndC").value||q("#reportStartC").value.slice(0,7)!==q("#reportMonthC").value||q("#reportEndC").value.slice(0,7)!==q("#reportMonthC").value)syncReportMonthC()}else if(!q("#reportStartC").value||!q("#reportEndC").value){q("#reportMonthC").value=q("#reportMonthB").value;localStorage.setItem("reportCMode","month");syncReportMonthC()}}
...
function setWorkspace(view){let button=q(`.nav button[data-view="${view}"]`);if(!button)return;qa(".nav button").forEach(x=>x.classList.remove('active'));button.classList.add('active');qa(".view").forEach(v=>v.classList.remove('active'));q("#"+view).classList.add('active');q("#pageTitle").textContent=button.textContent;q("#workspaceSelect").value=view}
