...
<main class="app">
  ...
  <section class="main">
    ...
    <section id="manual" class="view">
      ...
      <div class="panel"><div class="toolbar"><h2>Transaction history</h2></div><div id="manualHistory" class="empty">No manual check-ins saved yet.</div></div>
    </section>
    <section id="dailySales" class="view">
      <div class="panel"><div class="toolbar"><h2>Daily sales</h2></div><div class="filters"><button data-daily-range="today" class="active">Today</button><button data-daily-range="custom">Custom date</button><input id="dailyStartDate" type="date"><input id="dailyEndDate" type="date"></div><div id="cashFlowCards" class="cards"></div></div>
      <div class="panel"><div class="toolbar"><h2>Manual transactions</h2></div><div id="dailySalesTable" class="empty">No manual transactions in this date range.</div></div>
    </section>
...
<script>
const APP_VERSION="daily-sales-page-20260626";
...
function cashDepositActive(row){if((row.deposit_payment_method||"").toLowerCase()!=="cash")return false;let input=new Date((row.input_time||flagValue(row.flags,"INPUT_TIME")||"")+"");if(Number.isNaN(input.getTime()))return dailySalesMode!=="today";let now=new Date(),reset=new Date(now);reset.setHours(12,0,0,0);if(now<reset)reset.setDate(reset.getDate()-1);return input>=reset}
function renderDailySales(){let rows=manualTransactionRows(),totals={Cash:0,QR:0,Card:0,"Bank Transfer":0,"Online Booking":0},cashDeposit=0;rows.forEach(r=>{let room=paymentLabel((r.payment_method||"").toLowerCase()),kel=paymentLabel((r.kelestarian_payment_method||"").toLowerCase()),dep=paymentLabel((r.deposit_payment_method||"").toLowerCase());if(totals[room]!=null)totals[room]+=asMoney(r.price);if(totals[kel]!=null)totals[kel]+=asMoney(r.kelestarian);if(totals[dep]!=null)totals[dep]+=asMoney(r.deposit);if(cashDepositActive(r))cashDeposit+=asMoney(r.deposit)});let cards=q("#cashFlowCards");if(cards)cards.innerHTML=Object.entries(totals).map(([k,v])=>`<article class="metric"><span>${k}</span><strong>RM ${v.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong></article>`).join('')+`<article class="metric"><span>Cash Deposit</span><strong>RM ${cashDeposit.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong></article>`;let el=q("#dailySalesTable");if(!el)return;el.className=rows.length?'':'empty';el.innerHTML=rows.length?table(rows,['Input time','Room','Registration','Stay type','Duration','Check in','Check out','Room payment','Kelestarian payment','Deposit payment','Remark'],r=>`<tr><td>${html(formatDateTime(r.input_time||flagValue(r.flags,"INPUT_TIME")))}</td><td>${html(r.room_no)}</td><td>${html(r.registration||flagValue(r.flags,"REGISTRATION"))}</td><td>${html(r.stay_type_label||flagValue(r.flags,"STAY_TYPE"))}</td><td>${html(r.stay_progress)}</td><td>${html(String(r.check_in_date||r.date).split("-").reverse().join("/"))}</td><td>${html(String(r.check_out_date||"").split("-").reverse().join("/"))}</td><td>RM ${r.price||"0.00"} (${html(r.payment_method||"")})</td><td>RM ${r.kelestarian||"0.00"} (${html(r.kelestarian_payment_method||"")})</td><td>RM ${r.deposit||"0.00"} (${html(r.deposit_payment_method||"")})</td><td>${html(r.remark||flagValue(r.flags,"REMARK"))}</td></tr>`):'No manual transactions in this date range.'}
