...
def normalize_payment_method(value: str | None) -> str | None:
    ...
    if "cash" in text:
        return "Cash"
    if "qr" in text or "duitnow" in text:
        return "QR Payment"
    ...
    if "visa" in text or "master" in text or "american express" in text or "discover" in text or "card" in text:
        return "Bank"
    if "online" in text or "booking" in text or "ota" in text or "trip" in text or "xpdia" in text or "expedia" in text:
        return "Bank"
...
<script>
const APP_VERSION="payment-source-dashboard-20260626";
...
function bookingSource(value){let text=String(value||"").trim().toUpperCase();if(["BKING","OTA","TRIP","XPDIA","EXPEDIA"].includes(text))return text==="EXPEDIA"?"XPDIA":text;if(text==="WLKIN"||text==="WALK IN"||text==="WALK-IN")return "WALK IN";return text||"Unknown"}
function moneyBucket(method,source){let text=String(method||"").toLowerCase(),src=bookingSource(source);if(text.includes("cash"))return "Cash";if(text.includes("qr"))return "QR Payment";if(["BKING","OTA","TRIP","XPDIA"].includes(src))return "Bank";if(text.includes("bank")||text.includes("transfer")||text.includes("online")||text.includes("booking")||text.includes("card")||text.includes("visa")||text.includes("master")||text.includes("atm"))return "Bank";return text?method:"Unknown"}
function collectionBreakdown(rows){let map={};rows.filter(r=>!r.is_paid_continuation).forEach(r=>{let key=moneyBucket(r.payment_method,r.rate_type),source=bookingSource(r.rate_type);map[key]??={payment_method:key,bills:0,amount:0,kelestarian:0,sources:{}};map[key].bills++;map[key].amount+=asMoney(r.price);map[key].kelestarian+=asMoney(r.kelestarian);map[key].sources[source]=(map[key].sources[source]||0)+asMoney(r.price)});let rank=v=>v.startsWith("Cash")?0:v.startsWith("QR")?1:v.startsWith("Bank")?2:3;return Object.values(map).map(v=>{let sourceNames=Object.keys(v.sources).filter(s=>s&&s!=="Unknown").sort();return {...v,payment_method:v.payment_method==="Bank"&&sourceNames.length?`Bank (${sourceNames.join(", ")})`:v.payment_method}}).sort((a,b)=>rank(a.payment_method)-rank(b.payment_method)||b.amount-a.amount)}
...
const renderDashboardBase=render;render=function(){renderDashboardBase();let el=q("#analytics");if(el)el.innerHTML=el.innerHTML.replaceAll("Revenue by rate type","Revenue by booking/source type").replaceAll("Rate Type","Booking/Source Type")}
