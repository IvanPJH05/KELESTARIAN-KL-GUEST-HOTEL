...
def test_reporting_settings_include_default_contact_details():
    ...
    assert 'id="ownerSelect"' in PAGE
    assert 'value="__new__">+ Add new owner' in PAGE
    assert 'View owner' in PAGE
    assert 'selectedOwnerName' in PAGE
    assert 'No owner equity or drawings history yet.' in PAGE
    ...
    assert '<span>Imported stays</span>' not in PAGE
    assert '<span>Sales rows</span>' not in PAGE
    assert '#analytics>.cards{display:none}' in PAGE
    assert '#analytics tbody{display:grid' in PAGE
